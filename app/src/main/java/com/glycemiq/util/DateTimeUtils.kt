...
object DateTimeUtils {
    ...
    /** Guardar en Supabase: fecha y hora actual de México. */
    fun nowMexicoIso(): String =
        ZonedDateTime.now(MEXICO_ZONE).format(DateTimeFormatter.ISO_OFFSET_DATE_TIME)

    /** Convierte recorded_at de Supabase a millis para la app. */
    fun parseRecordedAtToMillis(recordedAt: String): Long =
        Instant.parse(recordedAt).toEpochMilli()
